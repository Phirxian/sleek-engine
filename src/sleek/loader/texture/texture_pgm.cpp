...
namespace sleek
{
    namespace loader
    {
        int texturemime_pgm::check_header(io::filereader *file) const noexcept
        {
        ...
        bool texturewriter_pgm::write(driver::texture *img, io::filewriter *file) const noexcept
        {
            #ifdef texture_loader_pgm_support
                if(img->getFormat() >= 5)
                    return false;
        ...
        bool texturemime_pgm::match(const std::string &filename) const noexcept
        {
